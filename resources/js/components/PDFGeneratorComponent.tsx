...
const PDFGeneratorComponent: React.FC<PDFGeneratorComponentProps> = ({
    ...
    const handleActionsDownload = async () => {
        ...
        try {
            const url = `/assessment/${assessment.id}/actions/pdf`;
            window.open(url, '_blank');
            toast({ title: t.success, description: t.downloadSuccess, variant: 'success' });
        } catch (error: any) {
                ...
                >
                    <div className="py-1">
                        ...
                        <div className="grid grid-cols-2 gap-1 p-2">
                            ...
                            {!isGuest && (
                                <ActionButton icon={<ListChecks />} label={t.actions} onClick={handleActionsDownload} loading={loadingStates.actions} />
                            )}
                        </div>
